...
- `mlx` (default, Apple Silicon) — uses `MLX_CHAT_MODEL` (default `mlx-community/Qwen3-30B-A3B-Instruct-2507-4bit`; on 128 GB machines set to `mlx-community/Qwen3-Next-80B-A3B-Instruct-8bit`, ~85 GB, MoE 3B-active, for the highest-quality option — needs `mlx-lm >= 0.27` for the `qwen3_next` architecture). Only one MLX model is served at a time; the "Inference Engine" dropdown can switch models at runtime (restarts `mlx_lm.server` with the new weights — no app restart needed), or set `MLX_CHAT_MODEL` to change the default. Boots `mlx_lm.server` as a subprocess on `MLX_SERVER_HOST:MLX_SERVER_PORT` (default `127.0.0.1:8081`) exposing an OpenAI-compatible API; LangChain connects via `ChatOpenAI`. KV prompt cache: `MLX_PROMPT_CACHE_SLOTS` (default `4`) and `MLX_PROMPT_CACHE_BYTES` (default `8_000_000_000`). Server readiness wait: `MLX_SERVER_STARTUP_TIMEOUT` (default `1200`s) — raise for large/first-time weights (the 80B is ~85 GB; a first-time switch that isn't pre-downloaded will otherwise time out and drop the Gradio connection). Pre-warmed at app startup.
